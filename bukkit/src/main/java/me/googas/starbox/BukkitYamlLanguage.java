...
public class BukkitYamlLanguage implements BukkitLanguage {
    ...
    public static BukkitYamlLanguage of(@NonNull Reader reader) {
        ...
        return new BukkitYamlLanguage(new Locale(Objects.requireNonNull(configuration.getString("language"), "There's no 'language' field")), configuration);
    }

    /**
     * Get a language from a {@link InputStream}. This will create a reader and use it in {@link #of(Reader)}
     *
    ...
    public BaseComponent[] get(@NonNull String key) {
        return BukkitUtils.getComponent(BukkitUtils.format(this.getRaw(key).orElse(key)));
    }

    @NonNull
    public BaseComponent[] get(@NonNull String key, @NonNull Map<String, String> map) {
        return BukkitUtils.getComponent(BukkitUtils.format(this.getRaw(key).orElse(key), map));
    }

    @NonNull
    public BaseComponent[] get(@NonNull String key, Object... objects) {
        return BukkitUtils.getComponent(BukkitUtils.format(this.getRaw(key).orElse(key), objects));
    }
}
