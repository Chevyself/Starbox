...
import me.googas.starbox.modules.channels.ConsoleChannel;
...
/**
 * Represents a channel which may only send data to players with an specific permission, this does also
 * include the console.
 */
public class PermissionChannel implements Channel {
    ...
    private Stream<? extends Player> filter() {
        return Bukkit.getOnlinePlayers().stream().filter(player -> player.hasPermission(this.permission));
    }
